...
Operator.prototype['getFivePixiver'] = function(callback){
	var BOOK =this.BOOK;
	var query = BOOK.find({});
	query.sort({follow:-1});
	query.limit(5);
...
Operator.prototype['getPixiver'] = function(number,callback){
	var BOOK =this.BOOK;
...
Operator.prototype['getFiveWork'] = function(callback){
	var BOOK =this.BOOK;
	var query = BOOK.find({});
	query.sort({praise:-1});
	query.limit(5);
